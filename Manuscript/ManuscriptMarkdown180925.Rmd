---
title: "Indicators for Effects on Mean and Variance in Projected Normal Regression Models for a Circular Outcome"

author: |
  | Jolien Cremers^[Corresponding author: j.cremers@uu.nl] $^1$, Daniel Hernandez-Stumpfhauser $^{2}$
...
#csl: apa.csl (Chicago style is default)
pandoc_args: --natbib
...
the 'intrinsic', 'wrapping' and 'embedding' approach [@mardia2000directional].
...
models for a circular outcome have been introduced [@fisher1992regression;
...
heterogeneous errors [@rivest2015general]. In a homogeneous error model predictor
...
first introduced by @presnell1998projected and adapted to the Bayesian context
by @nunez2011bayesian.

Several measures for assessing the effect on the mean of the outcome, a location
effect, in a PN regression model were introduced in @CremersMulderKlugkist2017.
...
where $\boldsymbol{u} = (\cos\theta, \sin\theta)^t$ and $r > 0$. The idea behind
...
use the approach used by @CremersMulderKlugkist2017 to solve the missing data
...
where $-\pi \leq \theta < \pi$, $\boldsymbol{\mu} = (\mu_1, \mu_2)^t \in
...
@kendall1974 for the exact form of this relation). A different way of
parameterizing a projected normal distribution can be found in
@wang2012directional.

```{r figPN, cache = TRUE, echo = FALSE, results = FALSE}

require(MASS)
require(circular)

y1 = mvrnorm(100, c(2, 2), diag(2))
y2 = mvrnorm(100, c(4, 4), diag(2))
y3 = mvrnorm(100, c(-2, 2), diag(2))
...
zeta1   <- sqrt(2^2 + 2^2)^2/4
zeta2   <- sqrt(4^2 + 4^2)^2/4
zeta3   <- sqrt(2^2 + 2^2)^2/4

tikz("Plots/PN.tex", standAlone =TRUE, height = 4, width = 8, pointsize = 10, engine = "pdftex")

par(mfrow=c(1,3),
    oma=c(0,0,0,0),
    mar=c(0,0,0,0))

plot(density.circular(theta_1, bw = zeta1, kernel = 'vonmises'),
     main = " ", ylab = " ", xlab = " ", shrink = 1.3, axes = FALSE)
axis.circular(at = 0, labels = "0")
plot(density.circular(theta_3, bw = zeta3, kernel = 'vonmises'),
     main = " ", ylab = " ", xlab = " ", shrink = 1.3, axes = FALSE)
axis.circular(at = 0, labels = "0")
plot(density.circular(theta_2, bw = zeta2, kernel = 'vonmises'),
     main = " ", ylab = " ", xlab = " ", shrink = 1.3, axes = FALSE)
axis.circular(at = 0, labels = "0")
...
tools::texi2dvi("Plots/PN.tex",pdf=TRUE)
...
in @CremersMulderKlugkist2017. Because the structure in (\ref{regression}) is
...
$SSDO$, has been introduced previously in @CremersMulderKlugkist2017. The second
...
In @CremersMulderKlugkist2017 it is outlined that the effects of a variable $x$
...
@CremersMulderKlugkist2017 introduced a measure, the signed shortest distance to
...
@CremersMulderKlugkist2017 make use of the highest posterior density (HPD) to
...
@CremersMulderKlugkist2017 perform a simulation for the $SSDO$. This simulation
...
```{r figLocAcc, cache = TRUE, dev = "tikz",fig.ext="svg", echo = FALSE, results = FALSE}

tikz("Plots/LocAcc.tex", standAlone =TRUE, height = 4, width = 8, pointsize = 10, engine = "pdftex")
...
plot(1, type="n", xlab="", ylab="", yaxt = "n", xaxt= "n", xlim=c(-5, 7), ylim=c(-5, 7), frame.plot = FALSE)

draw.circle(0, 0, 2, nv = 100, border = NULL, col = NA, lty = 1, lwd = 1)
...
arrows(x0 = 0, y0 = 0, x1 = -1*2, y1 = 1*2, length = 0.05, lwd = 3)
arrows(x0 = 0, y0 = 0, x1 = -2*2, y1 = 2*2, length = 0.05, lwd = 3)
arrows(x0 = 0, y0 = 0, x1 = -0.2*2, y1 = 0.2*2, length = 0.05, lwd = 3)
text(-0.2,2.5, label = "$(\\beta_0^{I}, \\beta_0^{II})$", cex = 1.2)
text(0.7,4, label = "$(\\beta_0^{I} + \\beta_1^{I}x_{max}, \\beta_0^{II} + \\beta_1^{II}x_{max})$", cex = 1.2)
text(4,0.4, label = "$(\\beta_0^{I} + \\beta_1^{I}x_{min}, \\beta_0^{II} + \\beta_1^{II}x_{min})$", cex = 1.2)
...
#draw lines
points(x*2, (y+1)*2, type = "l", lty = 1)
arrows(x0 = 0, y0 = 0, x1 = -1*2, y1 = 2*2, length = 0.05, lwd = 3)
arrows(x0 = 0, y0 = 0, x1 = -2*2, y1 = 3*2, length = 0.05, lwd = 3)
arrows(x0 = 0, y0 = 0, x1 = -0.2*2, y1 = 1.2*2, length = 0.05, lwd = 3)
text(0,4, label = "$(\\beta_0^{I}, \\beta_0^{II})$", cex = 1.2)
text(0.7,6, label = "$(\\beta_0^{I} + \\beta_1^{I}x_{max}, \\beta_0^{II} + \\beta_1^{II}x_{max})$", cex = 1.2)
text(4,2.5, label = "$(\\beta_0^{I} + \\beta_1^{I}x_{min}, \\beta_0^{II} + \\beta_1^{II}x_{min})$", cex = 1.2)
segments(x0 = 0, y0 = 0, x1 = cos(0.25*pi)^2*2, y1 = sin(0.25*pi)^2*2, length = 0.05, lwd = 3, lty = 3)
text(1,0, label = "$SDO$", cex = 1)

#Close the device
dev.off()

# Compile the tex file
tools::texi2dvi("Plots/LocAcc.tex",pdf=TRUE)
```
...
and are bounded between -1 and 1. From now on we will call these measures
...
From now on we will call this measure $\sin(\beta)$. A visual representation of
...
```{r figDet, cache = TRUE, dev = "tikz",fig.ext="svg", echo = FALSE, results = FALSE}

tikz("Plots/Determinant.tex", standAlone =TRUE, height = 4, width = 8, pointsize = 10, engine = "pdftex")
...
plot(1, type="n", xlab="", ylab="", yaxt = "n", xaxt= "n", xlim=c(-4, 6), ylim=c(-4, 6), frame.plot = FALSE)
...
points(x*2, (y+1)*2, type = "l", lty = 1)
arrows(x0 = 0, y0 = 0, x1 = -0.2*2, y1 = 1.2*2, length = 0.05, lwd = 3)
arrows(x0 = 0, y0 = 0, x1 = -2, y1 = 2, length = 0.05, lwd = 3)
#arrows(x0 = 0, y0 = 0, x1 = -0.4-2, y1 = 2.4 + 2, length = 0.05, lwd = 1, lty = 2)
text(1.2,2.5, label = "$(\\beta_0^{I}, \\beta_0^{II})$", cex = 1.2)
text(-3.5,2.5, label = "$(\\beta_1^{I}, \\beta_1^{II})$", cex = 1.2)
#text(1,4.5, label = "$(\\beta_0^{I} + \\beta_1^{I}, \\beta_0^{II} + \\beta_1^{II})$", cex = 1.2)

draw.arc(0, 0, 1, atan2(2.4, -0.4), atan2(2, -2))

text(-0.6, 1.1, label = "$\\beta$", cex = 1)
...
points(x*2, (y+1)*2, type = "l", lty = 1)
arrows(x0 = 0, y0 = 0, x1 = -0.2*2, y1 = 1.2*2, length = 0.05, lwd = 3)
arrows(x0 = -0.2*2, y0 = 1.2*2, x1 = -0.2*2 -2, y1 = 1.2*2 + 2, length = 0.05, lwd = 3)
arrows(x0 = 0, y0 = 0, x1 = -0.4-2, y1 = 2.4 + 2, length = 0.05, lwd = 1, lty = 2)
text(1.2,2.5, label = "$(\\beta_0^{I}, \\beta_0^{II})$", cex = 1.2)
#text(-3.5,2.5, label = "$(\\beta_1^{I}, \\beta_1^{II})$", cex = 1.2)
text(1,4.5, label = "$(\\beta_0^{I} + \\beta_1^{I}, \\beta_0^{II} + \\beta_1^{II})$", cex = 1.2)
...
# Compile the tex file
tools::texi2dvi("Plots/Determinant.tex",pdf=TRUE)
```
...
because in earlier simulations for the $SSDO$ in @CremersMulderKlugkist2017 only two
...
\sim N(0,1)$ similar to the simulation in @CremersMulderKlugkist2017. The relation
...
These population values were then multiplied by a multiplication factor between
...
simulation studies in @CremersMulderKlugkist2017 and @Cremers2018Assessing.
